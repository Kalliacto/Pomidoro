...
const buttonTodoAdd = document.createElement('button');
buttonTodoAdd.classList.add('todo__add');
buttonTodoAdd.textContent = 'Добавить новую задачу';
li.append(buttonTodoAdd);
// --------------------------------
const getTodo = () => {
...
export const initTodo = () => {
    ...
    buttonTodoAdd.addEventListener('click', () => {
        ...
        // Проверка null на и пустую строку
        if (title !== null && title.trim() !== '') {
            const todo = addTodo(title);
            createTodoListItem(todo);
            // Новую добавленную задачу делаем активной и запускаем отображение через showTodo();
            state.activeTodo = todo;
            showTodo();
        } else {
